...
canvasHeight = 500
fenêtre = tkinter.Tk()
fenêtre.title("Pong imposible V°écrant large")
canvas = tkinter.Canvas(fenêtre, width=canvasWidth, height=canvasHeight, bg="orange")
canvas.pack()
raquette = canvas.create_rectangle(0, 0, 40, 10, fill="white")
balle = canvas.create_oval(0, 0, 10, 10, fill="deeppink")
fenêtreOuverte = True
score = 0
compteRebonds = 0
...
def déplacer_balle():
    ...
    if mouvBalleY > 0 and basBalle > défHautRaquette and basBalle < défBasRaquette:
        ...
        if droiteBalle > gaucheRaquette and gaucheBalle < droiteRaquette:
            ...
            if compteRebonds == 1:
                compteRebonds = 0
                vitesseRaquette = vitesseRaquette + 4
                if mouvBalleX > 0:
                    mouvBalleX = mouvBalleX + 4
                else:
                    mouvBalleX = mouvBalleX - 4
                mouvBalleY = mouvBalleY - 4
